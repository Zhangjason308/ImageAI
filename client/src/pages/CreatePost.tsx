...
const CreatePost = () => {
  const navigate = useNavigate()
	const [form, setForm] = useState({name: '', prompt: '', photo: ''} as CreatePostFormType)
	const [generatingImg, setGeneratingImg] = useState(false)
	const [loading, setloading] = useState(false)
	
	async function generatingImage(): Promise<any> {
		if(form.prompt) {
			try {
		...
		<section className='max-w-7xl mx-auto'>
			...
			<form className='mt-16 max-w-3xl' onSubmit={handleSubmit} action="">
				...
				<div className='mt-5 flex gap-5'>
					<button type='button' onClick={generatingImage} className='text-white bg-green-400 font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center'>
							{generatingImg ? 'Generating...' : 'Generate'}
					</button>
				</div>

				<div className="mt-10">
					<p className='mt-2 text-[#666e75] text-[14px] italic'>Share you image with others in the community!</p>
